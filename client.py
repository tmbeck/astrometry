...
# https://raw.githubusercontent.com/dstndstn/astrometry.net/main/net/client/client.py
...
if __name__ == '__main__':
    ...
    parser.add_option('--upload-url', '-U', dest='upload_url', help='Upload a file at specified url')
    parser.add_option('--wait', '-w', dest='wait', action='store_true', help='After submitting, monitor job status')
    parser.add_option('--watch', '-W', help='Folder to watch for files to upload.', default=None)
    ...
    if opt.watch is not None:
        ...
        for changes in watch(opt.watch):
            for change in changes:
                if change[0] == Change.added:
                    if change[1].endswith('.jpg'):
                        opt.upload = change[1]
                        main_loop()
                        os.unlink(change[1])
